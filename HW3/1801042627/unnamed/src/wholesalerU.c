...
*/
int numberOfSemaphoresInitted = 0;
...
{   
   ...
   //printf("sharedMemoryName: %s\n",sharedMemoryName);
    ...
    //printf("sem_t: %d\n",sizeof(sem_t));
    int numberOfSemaphores = 15;
    int requiredSize =  sizeof(sem_t) * (numberOfSemaphores+1); // sizeof(char) * 6 + numberofsemaphores * sizeof(sem_t), first 32 bytes' first 6 bytes is only used, 
    ...
    //printf("ADDRESSES: %p %p",addr,sem_addr);
    ...
    if(sem_init(wholesaler_sem,1,1) == -1){ // between processes, initial value is 1
        perror("sem_init wholesaler1");
        waitForChildren();
        return -1;
    }
    numberOfSemaphoresInitted++;
    ...
    sem_t* pusher_sem = &sem_addr[1];
    if(sem_init(pusher_sem,1,1) == -1){ // between processes, initially it is 1 because it is used to make only one pusher work at the same time
        perror("sem_init pusher_sem");
        waitForChildren();
        return -1;
    }
    numberOfSemaphoresInitted++;
    ...
    if(createSemaphores(&sem_addr[2]) == -1)
    {
        perror("Could not create semaphores");
        waitForChildren();
        //sem_destroy(pusher_sem);
        //sem_destroy(wholesaler_sem);
        unlink_semaphores(sem_addr);
        removeSharedMemory(argv0,sharedFd,sharedMemoryName);
   ...
   /* sem_unlink(argv4);
    ...
    sem_t* helper_can_read = &sem_addr[12];
    if(sem_init(helper_can_read,1,0) == -1)
    {
    ...
    int wholesaler_id = forkWholesaler(sharedMemoryName,argv0);
    if(wholesaler_id == -1){
        waitForChildren();
        unlink_semaphores(sem_addr);
        //sem_destroy(pusher_sem);
        ...
        if ((bytesread = read(fd,arr,sizeof(char)*3)) > 0)
        {
            ...
            if(!isValid(arr,2)){
                ...
                /*if(sem_close(helper_can_read) == -1)
                    perror("sem_close helper_can_read");
...
int forkWholesaler(char * sharedMemoryName,char* programName)
{
    ...
    if(id_wholesaler == -1){
        ...
        char * argv[] = {"./helperU",programName,sharedMemoryName,NULL};
        execv(argv[0],argv);
    } else{
...
int forkChefs(char * sharedMemoryName,const char* programName){
    ...
    for(int i=0;i<NUMBER_OF_CHEFS;i++){
        ...
        if(id == 0){
            ...
            argv[4]: chef number
            */
            char * argv[] = {"./chefU",ingredientSemaphoreNamesForChefs[i],programName,sharedMemoryName,chefNumber,NULL};
            execv(argv[0],argv);
        } else{
...
void unlink_semaphores(sem_t *sem_arr){
    for (int i = 0; i < numberOfSemaphoresInitted; i++)
    {
...
int createSemaphores(sem_t* sem_arr){
    ...
    for (int i = 0; i < NUMBEROFINGREDIENTS +NUMBER_OF_CHEFS; i++)
    {
        int temp = sem_init(&sem_arr[i],1,0);
        if(temp == -1){
            perror("sem_init ingredient or chef");
            /*// destroy opened semaphores so far
            for(int j = 0;j<i;j++){
                unlink_semaphores(sem_arr);
            }
            waitForChildren();*/
            return -1;
        }
        ...
        if(id == 0){
           ...
           //printf("IIII:%d\n",i);
            if(i == 0){
                ...
                ingredient_1_semaphore = "7";
                ingredient_2_semaphore = "9";
                ingredient_3_semaphore = "b";
            } else if(i==1){
                //FLOUR
                snprintf(ingredientIndexes,4*sizeof(int),ingredientIndexesFormat,WALNUTS,MILK,SUGAR,FLOUR);
                ingredient_1_semaphore = "8";
                ingredient_2_semaphore = "9";
                ingredient_3_semaphore = "c";
            } else if(i == 2){
                //WALNUTS
                snprintf(ingredientIndexes,4*sizeof(int),ingredientIndexesFormat,FLOUR,MILK,SUGAR,WALNUTS);
                ingredient_1_semaphore = "8";
                ingredient_2_semaphore = "7";
                ingredient_3_semaphore = "a";
            }else { //if(i == 3)
                //SUGAR
                snprintf(ingredientIndexes,4*sizeof(int),ingredientIndexesFormat,FLOUR,MILK,WALNUTS,SUGAR);
                ingredient_1_semaphore = "c";
                ingredient_2_semaphore = "b";
                ingredient_3_semaphore = "a";
            }
            //buf[sizeof(sem_t)];
            char indexOfWantedIngredient[5];
            snprintf(indexOfWantedIngredient,2,"%d",i+3 );
            char *const argv[]= {"./pusherU",indexOfWantedIngredient,programName,sharedMemName,ingredientIndexes,ingredient_1_semaphore,ingredient_2_semaphore,ingredient_3_semaphore,NULL};
            execv(argv[0],argv);
        } else{
            childProcesses[activeChildProcesses++] = id;
        }
