...
// const char * ingredientNames[] = {"flour","milk","sugar","walnuts"};
    ...
    } else if(strcmp("/MS",argv[1]) == 0){
        waitedIngredient1 = ingredientNames[0];
        waitedIngredient2 = ingredientNames[3];
    } else if(strcmp("/MW",argv[1]) == 0){
        waitedIngredient1 = ingredientNames[0];
        waitedIngredient2 = ingredientNames[2];
    }else if(strcmp("/SW",argv[1]) == 0){
        waitedIngredient1 = ingredientNames[0];
    ...
    sem_t* ingredient = sem_open(argv[1],O_CREAT, S_IRUSR | S_IWUSR,0);
    ...
    sem_t* wholesaler_sem = sem_open(SEMFORWHOLESALER,O_CREAT , S_IRUSR | S_IWUSR,1);
